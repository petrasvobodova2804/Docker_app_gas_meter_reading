...
def draw_boxes(detections, image, colors):
    import cv2 # image dtype(uint8) (768,1024,3)
    for label, confidence, bbox in detections:
...
import numpy as np
def draw_boxes(detections, image, colors):
    image_pil = Image.fromarray(np.uint8(image))  # Convert NumPy array to PIL Image
    draw = ImageDraw.Draw(image_pil)
    font = ImageFont.load_default()  # You can use a different font if needed
    
    for label, confidence, bbox in detections:
        left, top, right, bottom = bbox2points(bbox)
        draw.rectangle([left, top, right, bottom], outline=colors[label], width=3)
        #draw.text((left, top - 15), "{} [{:.2f}]".format(label, confidence), fill=colors[label], font=font)
    
    return np.array(image_pil)  # Convert the modified PIL Image back to a NumPy array
...
if os.name == "posix":
    ...
    lib = CDLL("darknet.dll", RTLD_GLOBAL,winmode=0)
else:
